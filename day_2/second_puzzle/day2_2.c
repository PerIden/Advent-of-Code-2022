#include <stdio.h>
#include <stdlib.h>

enum Shape{
    ROCK = 1,
    PAPER = 2,
    SCISSORS = 3,
};

enum Result{
    LOSS = 1,
    DRAW = 2,
    WIN = 3,
};
/*
Hard coded lookup matrix.
Rows are the desired result (win/loss/draw)
Columns are which shape the elf chooses (A/B/C).
pick_shape[win/loss/draw][A/B/C] is will give you A, B or C.
*/
static const char pick_shape[3][3] = {
    'C','A','B',
    'A','B','C',
    'B','C','A',
};

static const int translate_shape[] = {
        ['A'] = ROCK,
        ['B'] = PAPER,
        ['C'] = SCISSORS,

        ['X'] = LOSS,
        ['Y'] = DRAW,
        ['Z'] = WIN,
};

int calculatePoints(enum Shape elf, enum Shape me){
    // draw
    if(elf == me){
        return me + 3;
    }
    // loss
    else if(elf == ROCK && me == SCISSORS || elf == SCISSORS && me == PAPER || elf == PAPER && me == ROCK){
        return me;
    }
    // win
    return me + 6;
}

int main(void){
    FILE *fptr = fopen("input.txt", "r");
    if(fptr == NULL){
        printf("Error! File not found.");
        exit(1);
    }
    int elf, me, result;
    int SIZE = 32;
    char buff[SIZE];
    int points = 0;
    while(fgets(buff, SIZE, fptr)){
        elf = (int)buff[0];
        result = (int)buff[2];
        me = pick_shape[translate_shape[elf] - 1][translate_shape[result] - 1];
        points += calculatePoints(translate_shape[elf],translate_shape[me]);
    }
    fclose(fptr); 
    printf("My total score is %d", points);
}